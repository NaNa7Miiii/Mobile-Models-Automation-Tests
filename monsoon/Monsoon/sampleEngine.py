...
class SampleEngine:
    def __init__(self, Monsoon, bulkProcessRate=128, errorMode=ErrorHandlingModes.full, calsToKeep=5):
        ...
        self.__channels = [self.__outputTimeStamp, self.__collectMainMeasurements, self.__collectUSBMeasurements,
                           self.__collectAuxMeasurements, self.__collectMainVoltage, self.__collectUSBVoltage]
        self.__channelnames = ["Time(ms)", "Main(mA)", "USB(mA)", "Aux(mA)", "Main Voltage(V)", "USB Voltage(V)"]
        self.__channelOutputs = [self.__mainCurrent, self.__usbCurrent, self.__auxCurrent, self.__mainVoltage,
                                 self.__usbVoltage]
    ...
    def __vectorProcess(self, measurements):
        ...
        if (self.__isCalibrated()):
            ...
            if (self.__channels[channels.MainCurrent]):
                ...
                mainCurrent = np.where(measurements[:, self.__mainFineIndex] < self.__fineThreshold, mainFinecurrents,
                                       mainCoarseCurrents)
            ...
            if (self.__channels[channels.USBCurrent]):
                ...
                usbCurrent = np.where(measurements[:, self.__usbFineIndex] < self.__fineThreshold, usbFineCurrents,
                                      usbCoarseCurrents)
            ...
            if (self.__channels[channels.AuxCurrent]):
                ...
                auxCurrent = np.where(measurements[:, self.__auxFineIndex] < self.__auxFineThreshold, auxFineCurrents,
                                      auxCoarseCurrents)
    ...
    def periodicStartSampling(self, calTime=1250):
        ...
        if (self.__CSVOutEnable):
            self.outputCSVHeaders()
        [[0 for _ in range(self.__packetSize + 1)] for _ in range(self.bulkProcessRate)]
        self.__startTime = time.time()
        self.monsoon.StartSampling(calTime, triggers.SAMPLECOUNT_INFINITE)
